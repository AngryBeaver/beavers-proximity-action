...
export class Activity {
    ...
    //should be a namespaced string that is valid as flag e.g. "beavers-proximity-action.secret-door
    get id(): string {
     ...
     */
    public isAvailable(actorId: string, hitArea: HitArea): boolean {
        for (const priority of PriorityTypeOrder) {
     ...
     */
    public async test(actorId: string, hitAreaData: bpa.HitArea, options: any): Promise<bpa.TestResult | null> {
        const hitArea = HitArea.create(hitAreaData);
    ...
    protected _getConfiguration(): bpa.ActivityConfiguration {
        const result: bpa.ActivityConfiguration = {}
